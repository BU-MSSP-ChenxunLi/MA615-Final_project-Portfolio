...
ui <- navbarPage("Explore Stock and Portfolio",
                 ...
                 tabPanel("Information Visualization",
                          br(),
                          sidebarPanel(
                              selectInput(
                                  inputId = "from",
                          ...
                          mainPanel(h3('Choose your Date to visualize the information!'),
                                    plotOutput('plot1'),
                                    plotOutput('plot4'),
                                    plotOutput('plot5'),
                                    plotOutput('plot2'))
...
server <- function(input,output){
    ...
    ##to1
    output$ui_to1 <- renderUI({
        selectInput(
            inputId = "to1",
            label = "To:",
            selected = "2020-11-30",
            choices = subset(sort(unique(stock_info$date)),sort(unique(stock_info$date))>input$from1)
        )
    })
    ...
    output$table <- DT::renderDataTable(DT::datatable({
        data <- filter(stock_info,date>=input$from1 & date<=input$to1)
    ...
    output$plot4=renderPlot(plot_4())
    
    
    ##plot5
    plot_5=reactive({
        x1 <- filter(stock_info,date>=input$from & date<=input$to)
        bar <- ggplot(x1, aes(x = date, y = close, 
                               open = open, high = high, low = low, close = close, 
                               group = symbol)) +
            geom_barchart() +
            labs(title = "Bar Chart for AMZN & GOOG & MSFT", 
                 y = "Closing Price", x = "Month") + 
            facet_wrap(~ symbol, ncol = 2, scales = "free_y") + 
            theme_tq()+
            theme(plot.title = element_text(hjust = 0.5, size = 20))
        ggpubr::ggarrange(bar,ncol = 1)
    })
    output$plot5=renderPlot(plot_5())
